...
namespace pid {
    ...
    struct datastructure_builder {
        ...
        template<typename T>
        using CacheType = std::map<T, decltype(std::declval<datastructure_builder>()(std::declval<T>())), std::less<>>;
        ...
        template<typename T>
        inline builder_offset<vector32<typename pid_type<T>::type>> operator()(const std::vector<T> &v) {
        ...
        operator()(const std::map<Key, Value> &m) {
            auto result{
                    b.add_map<typename pid_type<Key>::type, typename pid_type<Value>::type, std::uint32_t>(m.size())};
